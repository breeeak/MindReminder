...
describe('SpacedRepetitionAlgorithm', () => {
  ...
  describe('calculateNextReviewDate', () => {
    ...
    it('应对reviewCount为0抛出错误', () => {
      ...
      ).toThrow('Invalid reviewCount: 0. Must be an integer >= 1.')
    })
    ...
    it('应对frequencyCoefficient < 0.5抛出错误', () => {
      ...
      ).toThrow('Invalid frequencyCoefficient: 0.4. Must be between 0.5 and 1.5.')
    })

    it('应对frequencyCoefficient > 1.5抛出错误', () => {
      expect(() => 
        SpacedRepetitionAlgorithm.calculateNextReviewDate(baseDate, 1, 3, 1.6)
      ).toThrow('Invalid frequencyCoefficient: 1.6. Must be between 0.5 and 1.5.')
    })
